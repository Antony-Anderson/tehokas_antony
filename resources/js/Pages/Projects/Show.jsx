    ...
    return (
        <AppLayout>
            ...
            <div className="mb-6">
                ...
                {editingProject ? (
                    <form onSubmit={handleEditProject} className="bg-slate-900/50 border border-white/10 rounded-2xl p-6 mb-4 max-w-2xl">
                        <h3 className="text-sm font-medium text-slate-400 mb-4">Editar Projeto</h3>
                        <div className="space-y-3">
                            <input
                                type="text"
                                value={data.title}
                                onChange={e => setData('title', e.target.value)}
                                className="w-full bg-slate-950/50 border border-white/10 rounded-xl py-2.5 px-4 text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 text-sm"
                                placeholder="Título do projeto"
                            />
                            <textarea
                                value={data.description}
                                onChange={e => setData('description', e.target.value)}
                                className="w-full bg-slate-950/50 border border-white/10 rounded-xl py-2.5 px-4 text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 text-sm resize-none"
                                rows={2}
                                placeholder="Descrição"
                            />
                            <div className="flex gap-2">
                                <button type="button" onClick={() => setEditingProject(false)} className="px-4 py-2 rounded-xl border border-white/10 text-sm text-slate-400 hover:bg-white/5 transition-all">
                                    Cancelar
                                </button>
                                <button type="submit" disabled={processing} className="px-4 py-2 rounded-xl bg-indigo-600 text-sm text-white hover:bg-indigo-500 transition-all disabled:opacity-60">
                                    Salvar
                                </button>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-4 mt-3 text-xs text-slate-600">
                                ...
                                {project.overdue_count > 0 && (
                                    <span className="text-rose-400">{project.overdue_count} atrasadas</span>
                                )}
                            </div>
                        </div>
            ...
            <div className="flex items-center gap-3 mb-6 flex-wrap">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500">Filtrar por prioridade:</span>
                </div>

                <select
                    value={filterPriority}
                    onChange={e => setFilterPriority(e.target.value)}
                    className="text-xs bg-slate-900 border border-white/10 rounded-lg py-1.5 px-3 text-slate-300 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 cursor-pointer"
                >
